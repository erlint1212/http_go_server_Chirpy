...
// ripped from https://medium.com/@itachisasuke/implementing-a-search-engine-in-golang-trie-data-structure-c45152ddda24
...
func (t *Tries) FindMatches(document string) (matches []string) {
    matches := []string{}
    for i:=0; i<len(document); i++ {
        level := t.RootNode
        for j:=0; i<len(document); j++ {
            ch := document[j].Char
            if !slices.Contains(ch, document) {
                break; 
            }
            level = level[ch]
        }
